...
**Основные таблицы:**
* `stores` - Информация о магазинах и их местоположении
* `sales` - Еженедельные записи о продажах
* `economic_factors` - CPI, уровень безработицы
* `external_factors` - Температура, цены на топливо, праздничные флаги
...
**Еженедельные продажи по магазинам с экономическим контекстом:**
...
SELECT 
    ...
    f.fuel_price
FROM sales s
INNER JOIN economic_factors e ON s.sales_date = e.date
INNER JOIN external_factors f ON s.sales_date = f.date
WHERE s.sales_date BETWEEN '2010-01-01' AND '2012-12-31'
ORDER BY s.sales_date, s.store_id;
```
...
* **Python** (pandas, numpy) – Очистка, трансформация и анализ данных
...
Данный проект доступен для образовательных и аналитических целей.
